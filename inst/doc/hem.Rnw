...
\[
F_i = \sum_{j=1}^C  \frac{ w_{i,j} (\bar{\mu}_{i,j}-\bar{\bar{\mu}}_{i})^2}
...
More differentially expressed genes have larger $F$-scores; hence, 
a desired number of genes with large $F$-scores are selected. 
The false discovery rate (FDR) can be used to determine a threshold value of $F$, as described in Section 4.  
...
In the \Rfunction{hem} function, the argument  \Rfunarg{n.layer=2} indicates two-layer EM, and 
...
> pbrain.hem <- hem(pbrain.nor, n.layer=2, design=design)    #fit EM
> pbrain.hem$F                                               #F-scores


\end{verbatim}
$F$-scores are saved into \Robject{pbrain.hem\$F} in the order of original data, 
...
Suppose $F$-statistics are computed from raw data, 
and $F^0$-statistics from generated null data. 
...
\end{equation}
where 
$\bar{R}^0(\Delta) = \# \{F^0_{ib}| F^0_{ib} \ge \Delta, \; i=1,\ldots,G, b=1,\ldots,B  \} / B$ is 
the average number of significant genes in null data,
and $R(\Delta) = \# \{F_i| F_i \ge \Delta, \; i=1,\ldots,G  \}$ is the number of significant genes in raw data.
The estimate of a correction factor with the $\lambda$-quantile $m_\lambda$ of $F^0_{ib}$ is
$\hat{\pi}_0(\lambda) = \# \{F_i| F_i \le m_\lambda  \}/\# \{F^0_{ib}| F^0_{ib} \le m_\lambda  \}$,
which is required because of the different numbers of true insignificant genes in raw data and null data.  

The function \Rfunction{hem.fdr} provides FDRs at all $F$ values and $F$ critical values for given target FDRs.  
...
$
F_i = \sum_{j=1}^C w_{i,j} (\bar{\mu}_{i,j}-\bar{\bar{\mu}}_{i})^2 /
                             \bar{\sigma}^2_{\epsilon_{i,j}}.
$


The use of one-layer EM is demonstrated with the mouse B-cell development data set,
...
Example 5.1 shows how to fit one-layer EM with $\mbox{Gamma}(\alpha_\epsilon,\beta_\epsilon)$ prior
for variance parameter, $\sigma_{\epsilon_{i,j}}^{-2}$.
...
> mubcp.hem <- hem(mubcp.nor, n.layer=1,design=design)       #fit EM
...
In Example 4.2, the arguments \Rfunarg{method.var.t="neb"}   
...
\begin{itemize}
\item{\Rfunarg{fdr}}: $F$-scores and corresponding FDRs.
\item{\Rfunarg{pi0}}: estimated proportion of true negatives.
\item{\Rfunarg{F.null}}: $F$-scores from null data.
